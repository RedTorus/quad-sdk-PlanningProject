...
namespace planning_utils
{
  ...
  std::unique_ptr<CollisionChecker> collision_checker; // Define as a unique_ptr
  ...
  void initializeCollisionChecker(ros::NodeHandle& nh, const std::string& yaml_file_path) {
    ...
    collision_checker = std::make_unique<CollisionChecker>(bounding_boxes); // Initialize dynamically
  }
                    ...
                    int phase, double &max_valid_z)
  {
    ...
    for (int i = 0; i < planner_config.num_collision_points; i++)
    {
      ...
      geometry_msgs::Point collision_pt;
